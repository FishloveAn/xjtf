...
> 配套清单：`docs/项目文档/02-设计/02-设计-美术-替换清单-M3.md`（替换对象/优先级/技术约束/获取步骤）。

## 已入库

| 日期 | 文件名（入库后） | 素材原名 | 作者 | 来源链接 | 许可 | 用途 |
|---|---|---|---|---|---|---|
| 2026-08-06 | `assets/models/characters/char_zombie_common_01.glb` | Zombie_Basic | Quaternius | https://quaternius.com/packs/zombieapocalypsekit.html | CC0 | 普通丧尸瘦型（剥骨静态，2026-08-07 重建为 1200 tris） |
...
| 2026-08-06 | `assets/models/characters/char_player_01.glb` | BlueSoldier_Male（剥骨静态） | Quaternius | https://quaternius.com/packs/ultimateanimatedcharacter.html | CC0 | 玩家 Body（P2；2994 tris，2 材质，入库 GLB 高 1.80m） |
| 2026-08-06 | `assets/models/weapons/wep_pistol_01.glb` | Pistol_1 | Quaternius | https://quaternius.com/packs/ultimateguns.html | CC0 | 手枪（P2；1040 tris，长 0.25m；场景旋转 Y 180° 后枪口朝 -Z 并对齐 z=-0.40） |
| 2026-08-06 | `assets/models/weapons/wep_shotgun_01.glb` | Shotgun_1 | Quaternius | https://quaternius.com/packs/ultimateguns.html | CC0 | 霰弹枪（P2；1270 tris，长 0.90m；场景枪口对齐 z=-0.45） |
| 2026-08-06 | `assets/models/weapons/wep_rifle_01.glb` | AssaultRifle_1 | Quaternius | https://quaternius.com/packs/ultimateguns.html | CC0 | 步枪（P2；1248 tris，长 0.80m；场景枪口对齐 z=-0.45） |
| 2026-08-06 | `assets/models/weapons/wep_smg_01.glb` | SubmachineGun_1 | Quaternius | https://quaternius.com/packs/ultimateguns.html | CC0 | 冲锋枪（P2；1164 tris，长 0.60m；场景枪口对齐 z=-0.45） |
...
- **减面处理**：pymeshlab Quadric Edge Collapse，最终采用 1200 tris。原 999 tris 版本在激进减面后出现头部和肢体散架，已于 2026-08-07 从剥骨静态源重新构建，并通过渲染预览确认轮廓、肢体和贴图完整。
...
- **僵尸外观**：绿皮病态丧尸（Quaternius 原画），符合"病态"丧尸语义；缩放已烘焙进 GLB，源模型高度约 1.73m，对齐 1.7m 胶囊碰撞。
...
- **玩家 Body**：使用 Quaternius Ultimate Animated Character（CC0）的 `BlueSoldier_Male`，剥骨静态后为 2994 tris。2026-08-07 将原 6 个纯色材质合并为“主体+面部”2 个材质，并把 0.578 缩放烘焙进 GLB；入库 GLB 高 1.80m。`player.tscn/Body` 使用 scale=1、position y=0，使脚底/头顶分别对齐 y=0/1.80m；场景绕 Y 旋转 180°，把模型正面从局部 +Z 校正到 Godot 前方 -Z。
- **武器**：Kenney FPS Kit（`kenney.nl/assets/fps-kit`）**官网已下架 404**，wayback 多个 API 端点（CDX、available、calendarcaptures、id_ 抓取）**均无该 URL 存档**（calendarcaptures 2022 年矩阵全 null/{}，CDX kenney.nl/assets/fps-kit 返回空 []）。art-director 文档 §1.2 W3/W4 明确授权替代方案："Kenney FPS Kit（不足用 Quaternius Weapon 包或 Poly Pizza 补，只取 CC0）"——故采用 Quaternius Ultimate Guns（CC0）。该包仅含 .blend/.fbx/.obj 格式（无 glTF），使用 `pymeshlab` 加载 FBX→OBJ → `trimesh` 加载 OBJ→GLB 转换。脚本：_tmp_art_xjtf_dl/fbx_to_glb.py（Pistol 0.25m/Shotgun 0.9m/AssaultRifle 0.8m/SubmachineGun 0.6m 长度校准，最长轴对齐 Z，GLB 局部 +Z 端为枪口）；四个武器场景的模型实例统一绕 Y 旋转 180°，使枪口朝 Godot 前方 -Z，并分别对齐 `muzzle_offset`。Google Drive 多次限流（"many accesses"）下，部分 FBX 反复重试下载成功。模型无原始贴图/顶点色，转换后各 GLB 内嵌 1 个纯色材质。
- **第一人称手臂降级**：Kenney FPS Kit 通常含 view 模型带手臂，官网下架后无 view/world 区分来源。第一人称玩家看不到自己 Body（相机在 Head y=1.6，第三人称 Body 仅队友看），且 FPS Kit 已无 view 资源可下载，**只显示武器，不显示手臂**（按任务约束"第一视角手臂若素材无现成则降级"）。四把枪复用同一套 CC0 模型：第一人称 `ViewMesh` 挂在本地 Camera 下、使用独立渲染层且不投阴影；供其他玩家查看的 `WorldMesh` 保留在 WeaponPivot 各逻辑武器下，继续由 `_set_active_weapon` 切换。
